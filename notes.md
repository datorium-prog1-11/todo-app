...
## Lēmumu pieņemšana ar `if`:

- Lai mēs varētu apstrādāt dažādas situācijas mūsu programmā
- Lēmumu var pieņemt ar `if`
- Strādā ar `boolean` vērtībām (true/false)

```js
let skaitlis = 5;

if (skaitlis > 3) {
  console.log("skaitlis ir lielāks par 3");
} else {
  console.log("skaitlis ir mazāks par 3");
}
```

```js
const apstiprinajums = confirm("Vai piekrīti?");

if (apstiprinājums) {
  console.log("Tu piekriti");
} else {
  console.log("Tu nepiekriti");
}
```
